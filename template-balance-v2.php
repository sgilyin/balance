...
function getQR($payCode, $sum, $address) {
    $param = array(
        'ST00012',
        'Name=' . SBRF_QR_NAME,
        'PersonalAcc=' . SBRF_QR_PERSONALACC,
        'BankName=' . SBRF_QR_BANKNAME,
        'BIC=' . SBRF_QR_BIC,
        'CorrespAcc=' . SBRF_QR_CORRESPACC,
        'Category=' . SBRF_QR_CATEGORY,
        'TechCode=' . SBRF_QR_TECHCODE,
        'PayeeINN=' . SBRF_QR_PAYEEINN,
        'KPP=' . SBRF_QR_KPP,
        'persAcc=' . $payCode,
        'Purpose=Оплата абонентской платы по л/с ' . $payCode,
        'Sum=' . floatval($sum * 100),
        'payerAddress=' . $address,
    );
    $linkQr = 'http://chart.apis.google.com/chart?cht=qr&chs=300x300&chl=' . urlencode(implode('|', $param));
    return $linkQr;
}
    ...
    switch ($cid) {
        ...
        default:
            ...
            $html = sprintf('<p>Абонент: %s</p><p>Адрес: %s</p><p>Статус 
                ...
                для оплаты (лицевой счет) %s</p><p>Для пополнения счета 
                отсканируйте QR-код в приложении вашего банка</p>
                <p><img src="%s" alt="QR-Code"></p>', $htmlSubscriber, 
...
- Через <a href="https://vk.com/fialkatv">официальную группу вконтакте</a>.<br>
